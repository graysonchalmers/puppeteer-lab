...
| 3 | Face into `useTracker`; `useFaceTracker` becomes the adapter; `FaceDemo` onto the shared render loop | S | Host: Face Puppet works; enabling `hands` and `face` together in Hand Telemetry (dev-only toggle) shows both in one loop at 30 fps or better. Landed 2026-09-22 via Face Puppet (face-only adapter dropped: no consumers left). Host check pending. |
