...
const lipFlagsFull = (ts) => ts.map((t) => {
  const c = [0, 1].map((a) => t.reduce((s, i) => s + P(i)[a], 0) / 3);
  return inPoly(c, LIPS_OUTER_POLY) ? 1 : 0;
});
 ...
 * ${fullTris.length} FULL triangles.
 */
...
export const FACE_TRI_IS_LIP_FULL: readonly (0 | 1)[] = ${arr(lipFlagsFull(fullTris))};
