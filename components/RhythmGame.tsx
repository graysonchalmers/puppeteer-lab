...
import { Play, RefreshCw, VideoOff, Hand, Sparkles, ArrowLeft } from 'lucide-react';
...
const RhythmGame: React.FC<RhythmGameProps> = ({ onBack }) => {
  ...
  // Stop audio when component unmounts (user hits back)
  useEffect(() => {
    return () => {
      if (audioRef.current) {
        audioRef.current.pause();
  ...
  return (
    <div className="relative w-full h-full bg-black overflow-hidden font-sans">
      ...
      <div className="absolute top-4 left-4 z-50 flex items-center gap-3">
          <button 
            onClick={onBack}
            className="flex items-center gap-2 bg-white/10 hover:bg-white/20 px-3.5 py-1.5 rounded-lg border border-white/15 transition-all text-xs font-mono text-white cursor-pointer backdrop-blur-md"
          >
      ...
      <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-6 z-10">
          ...
          {/* HUD (Top) */}
          <div className="flex justify-between items-start text-white w-full mt-12 md:mt-0">
             {/* Health Bar */}
